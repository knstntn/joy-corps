...
def extend(x, y, path, extended_folder, names):
    ...
    for m in range(0,len(x)):
        if m<1 :
            ...
            z.append(y[nn-1,:])
    np.savetxt(extended_folder + '/' + names, z, fmt='%s', delimiter=',') # Write to the folder
...
def merge(path, name):
    ...
    if name_b == 'eyes':
        ...
        np.savetxt(input_folder + '/' + name_c, parts, fmt='%s', delimiter=',') # Write to the folder
        #sio.savemat('%s/%s.mat'%(input_mat_folder, name_c), parts, )
...
if __name__ == '__main__':
    ...
    if '/data/train' not in path:
        ...
        for file in files:   # Read the file
            ...
            np.savetxt(output_folder + '/' + name, label, fmt='%s', delimiter=',')  # Write to the folder
        ...
        np.savetxt('%s/input.csv'%path.split('train')[0], xtext, fmt='%s', delimiter=',')  # Write to the folder
        np.savetxt('%s/output.csv'%path.split('train')[0], ytext, fmt='%s', delimiter=',')  # Write to the folder
        print('Finished')
